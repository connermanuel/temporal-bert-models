...
@pytest.fixture
def random_input():
    return torch.rand(B_SIZE, SEQ_LEN, H_SIZE)
...
def test_composition_as_identity_loss(orth_linear, orth_linear_2, eye):
...
def test_batched_functionality(orth_linear, random_input):
    """Verifies that a batched orthogonal linear layer is the same as separate linear layers."""
    out_batched = orth_linear(random_input)

    linear_layers = [torch.nn.Linear(H_SIZE, H_SIZE, bias=False) for _ in range(B_SIZE)]
